...
class GIOU(nn.Module):
    ...
    def forward(self,preds, priors,targets,pos):
        ...
        b2_x1, b2_y1,b2_x2,b2_y2 = boxes_t[:,0], boxes_t[:,1],boxes_t[:,2],boxes_t[:,3]

        inter = (torch.min(b1_x2,b2_x2) - torch.max(b1_x1,b2_x1)) * \
                (torch.min(b1_y2,b2_y2) - torch.max(b1_y1,b2_y1))
            
        w1,h1 = b1_x2 - b1_x1, b1_y2 - b1_y1
        w1,h1 = b2_x2 - b2_x1, b2_y2 - b2_y1
        ...
        if self.loss_type == "ciou":
            v = (4/math.pi ** 2)*torch.pow(torch.atan(w2/h2) - torch.aten(w1/h1),2)
            with torch.no_grad():
                alpha = v/(1-iou+v)
...
class MultiBoxLoss2(nn.Module):
    ...
    def forward(self, predictions, priors, targets):
        ...
        pos_idx2 = pos_mafa.unsqueeze(pos.pos_mafa.dim().expand_as(landm_data))
